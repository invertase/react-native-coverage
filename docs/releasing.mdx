...
default `GITHUB_TOKEN` cannot bypass required pull requests, so the release will fail at the
git step unless the workflow runs with a token allowed to bypass protection. Options:

- Add the release bot (or a GitHub App / fine-grained PAT) to **"Allow specified actors to
  bypass required pull requests"** on the `main` ruleset, and expose that token to the workflow.
